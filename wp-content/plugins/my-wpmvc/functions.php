...
function sanitize_order_request( $request_data ) {
	...
	if ( $request_data['pickup'] ) {
        if ( isset($request_data['address_key']) ) {
	        $key = (int) $request_data['address_key'];
	        if ( ! ( $key >= 0 && array_key_exists( $key, get_option( 'addresses_for_pickup', [] ) ) ) ) {
		        $form_errors['address_key'] = 'Ошибка!';
	        }
        }
	} else {
        if ( isset($request_data['city_key']) ) {
	        $key = (int) $request_data['city_key'];
	        if ( ! ( $key >= 0 && array_key_exists( $key, get_option( 'cities_for_delivery', [] ) ) ) ) {
...
function my_wpmvc_options_page_output() {
	if ( ! empty( $_POST ) ) {
		...
		} elseif ( isset( $_POST['delete_option_city'] ) || isset( $_POST['delete_option_address'] ) ) {
			OptionsController::delete();
		}
	}
	require_once ABSPATH . 'wp-content/plugins/my-wpmvc/assets/views/admin/options_page.php';
}
